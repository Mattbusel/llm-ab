...
A/B test LLM prompts and models with statistical significance.

## Features
- Run two variants against a test set and score outputs
- Welch's t-test (two-tailed) for significance testing
- Cohen's d effect size
- Custom scoring functions (exact match, contains, LLM-as-judge, etc.)
...
llm::Variant control   = { "control",   "Answer: {input}", "gpt-4o-mini", api_key };
llm::Variant treatment = { "treatment", "Expert answer: {input}", "gpt-4o-mini", api_key };
auto result = llm::run_ab_test(control, treatment, samples);
